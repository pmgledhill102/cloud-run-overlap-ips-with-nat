#!/usr/bin/env bash
#
# direct-vpc-egress/setup-connectivity.sh — Set up HA VPN, BGP, Hybrid NAT, Public NAT, and ILB (idempotent)
#
...
echo "=== Setup Connectivity (Direct VPC Egress) for project: ${PROJECT_ID} ==="
echo "Region: ${REGION}"
echo ""
...
# Each spoke: advertise routable and pnat subnets
for spoke_num in 1 2; do
  spoke="spoke-${spoke_num}"
  router="vpn-router-${spoke}"
  ranges="10.${spoke_num}.0.0/22,172.16.${spoke_num}.0/24"
...
# ============================================================
echo ""
echo "=== Connectivity setup complete (Direct VPC Egress) ==="
echo ""
